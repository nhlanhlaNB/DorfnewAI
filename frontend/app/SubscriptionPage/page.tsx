...
const PaymentForm: React.FC<PaymentFormProps> = ({ plan, priceId, onClose }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.paymentForm}>
      ...
      <CardElement options={{ style: { base: { color: "#ffffff", fontSize: "16px" } } }} />
  ...
  return (
    <div className={styles.pageWrapper}>
        ...
        <div className={styles.modal}>
          <div className={styles.modalContent}>
            <Elements stripe={stripePromise}>
              <PaymentForm plan={selectedPlan} priceId={selectedPriceId} onClose={closeModal} />
            </Elements>
          </div>
        </div>
      )}
      <footer className={styles.footer}>
        <p className="mb-0">
          <i className="fas fa-copyright"></i> 2025 DorfNewAI. All rights reserved.
        </p>
      </footer>
    </div>
  );
